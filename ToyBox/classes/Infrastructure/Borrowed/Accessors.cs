...
namespace ToyBox {
    ...
    public delegate TResult FastInvoker<in TClass, in T1, in T2, in T3, out TResult>(TClass target, T1 arg1, T2 arg2, T3 arg);
    ...
    public class Accessors {
        ...
        public static FastGetter<TClass, TResult> CreateGetter<TClass, TResult>(string name) {
            ...
            if (!typeof(TResult).IsAssignableFrom(memberType)) {
                throw new InvalidCastException(
                    $"Cannot cast field type {typeof(TResult).Name} as {memberType} for class {typeof(TClass).Name} {memberTypeName} {name}");
            }

            var handler = isProp ? FastAccess.CreateGetterHandler<TClass, TResult>(propInfo) : FastAccess.CreateGetterHandler<TClass, TResult>(fieldInfo);
        ...
        public static FastSetter<TClass, TValue> CreateSetter<TClass, TValue>(string name) {
            ...
            var handler = isProperty ? FastAccess.CreateSetterHandler<TClass, TValue>(propertyInfo) : FastAccess.CreateSetterHandler<TClass, TValue>(fieldInfo);
        ...
        public static FastInvoker CreateInvoker(Type classType, string name, Type resultType, params Type[] parameters) {
            ...
            if (methodInfo == null) {
                string argString = string.Join(", ", parameters.Select(t => t.ToString()));
