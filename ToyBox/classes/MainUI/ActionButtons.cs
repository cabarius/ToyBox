...
namespace ToyBox
{
    public class NamedTypeFilter
    {
        ...
        protected NamedTypeFilter(string name,
                                  Type type,
                                  Func<SimpleBlueprint, bool> filter = null,
                                  Func<SimpleBlueprint, string> collator = null,
                                  Func<IEnumerable<SimpleBlueprint>> blueprintSource = null)
        {
            this.name = name;
            this.type = type;
    ...
    {
        public NamedTypeFilter(string name,
                               Func<TBlueprint, bool> filter = null,
                               Func<TBlueprint, string> collator = null,
                               Func<IEnumerable<SimpleBlueprint>> blueprintSource = null)
            : base(name, typeof(TBlueprint), null, null, blueprintSource)
        {
            if (filter != null)
            {
                this.filter = (bp) => filter((TBlueprint)bp);
            }

            if (collator != null)
            {
                this.collator = (bp) => collator((TBlueprint)bp);
            }
        }
    }
