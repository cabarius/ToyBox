...
namespace ToyBox
{
    public class PartyEditor
    {
        ...
        {
            if (partyFilterPlayer != Game.Instance.Player)
            {
            ...
            {
                partyFilterChoices = new[]
                {
                    new NamedFunc<List<UnitEntityData>>("Party",
                                                        () => Game.Instance.Player.Party),
                    new NamedFunc<List<UnitEntityData>>("Party & Pets",
                                                        () => Game.Instance.Player.m_PartyAndPets),
                    new NamedFunc<List<UnitEntityData>>("All",
                                                        () => Game.Instance.Player.AllCharacters),
                    new NamedFunc<List<UnitEntityData>>("Active",
                                                        () => Game.Instance.Player.ActiveCompanions),
                    new NamedFunc<List<UnitEntityData>>("Remote",
                                                        () => Game.Instance.Player.m_RemoteCompanions),
                    new NamedFunc<List<UnitEntityData>>("Custom",
                                                        PartyUtils.GetCustomCompanions),
                    new NamedFunc<List<UnitEntityData>>("Pets",
                                                        PartyUtils.GetPets),
                    new NamedFunc<List<UnitEntityData>>("Nearby",
                                                        () =>
                                                        {
                                                            ...
                                                            return player == null
                                                                ...
                                                                : GameHelper.GetTargetsAround(GameHelper.GetPlayerCharacter().Position, nearbyRange, false)
                                                                            .ToList();
                                                        }),
                    new NamedFunc<List<UnitEntityData>>("Friendly",
                                                        () => Game.Instance.State.Units.Where(u => u != null && !u.IsEnemy(GameHelper.GetPlayerCharacter()))
                                                                  .ToList()),
                    new NamedFunc<List<UnitEntityData>>("Enemies",
                                                        () => Game.Instance.State.Units.Where(u => u != null && u.IsEnemy(GameHelper.GetPlayerCharacter()))
                                                                  .ToList()),
                    new NamedFunc<List<UnitEntityData>>("All Units",
                                                        () => Game.Instance.State.Units.ToList()),
                };
            }
        ...
        public static List<UnitEntityData> GetCharacterList()
        {
            ...
            return filterChoices?[Main.settings.selectedPartyFilter].func();
        }
        ...
        public static void OnGUI()
        {
            ...
            foreach (UnitEntityData ch in characterList)
            {
                ...
                IEnumerable<Spellbook> enumerable = spellbooks.ToList();
                int spellCount = enumerable.Sum(sb => sb.GetAllKnownSpells().Count());
                ...
                using (UI.HorizontalScope())
                {
                    ...
                    if (UI.DisclosureToggle($"{classData.Count} Classes", ref showClasses))
                    {
                        if (showClasses)
                        {
                            ...
                            selectedToggle = ToggleChoice.Classes;
                            Main.Log($"selected {ch.CharacterName}");
                        }
                    ...
                    if (UI.DisclosureToggle("Facts", ref showFacts, 125))
                    {
                        ...
                        {
                            selectedCharacter = ch;
                            selectedToggle = ToggleChoice.Facts;
                        }
                        else { selectedToggle = ToggleChoice.None; }
                    }
                    ...
                    if (UI.DisclosureToggle("Buffs", ref showBuffs, 125))
                    {
                        ...
                        {
                            selectedCharacter = ch;
                            selectedToggle = ToggleChoice.Buffs;
                        }
                        else { selectedToggle = ToggleChoice.None; }
                    }
                    ...
                    if (UI.DisclosureToggle("Abilities", ref showAbilities, 125))
                    {
                        ...
                        {
                            selectedCharacter = ch;
                            selectedToggle = ToggleChoice.Abilities;
                        }
                        else { selectedToggle = ToggleChoice.None; }
                    }
                    ...
                    if (enumerable.Any())
                    {
                        ...
                        if (UI.DisclosureToggle($"{spellCount} Spells", ref showSpells))
                        {
                            ...
                            {
                                selectedCharacter = ch;
                                selectedToggle = ToggleChoice.Spells;
                            }
                            else { selectedToggle = ToggleChoice.None; }
                        }
                    }
                ...
                if (ch == selectedCharacter && selectedToggle == ToggleChoice.Classes)
                {
                    ...
                    using (UI.HorizontalScope())
                    {
                        ...
                        {
                            UI.Space(50);
                            UI.Label("Experimental Preview ".magenta());
                        }
                    }
#endif
                    UI.Div(100, 20);
                    ...
                    else
                    {
                        ...
                        using (UI.HorizontalScope())
                        {
                            ...
                            UI.Label("This directly changes your character level but will not change exp or adjust any features associated with your " +
                                     "character. To do a normal level up use +1 Lvl above".green());
                        }
                        ...
                        using (UI.HorizontalScope())
                        {
                            ...
                            UI.Label("This sets your experience to match the current value of character level.  It does not trigger any level ups or " +
                                     "adjusts any character features. To do a normal level up use +1 Lvl above".green());
                        }
                        ...
                        using (UI.HorizontalScope())
                        {
                            ...
                            UI.Label("This directly changes your mythic level but will not adjust any features associated with your character. To do a " +
                                     "normal mythic level up use +1 my above".green());
                        }
                        ...
                        foreach (var cd in classData)
                        {
                            ...
                            using (UI.HorizontalScope())
                            {
                                ...
                                int maxLevel = cd.CharacterClass.Progression.IsMythic ? 10 : 20;
                ...
                if (ch == selectedCharacter && selectedToggle == ToggleChoice.Stats)
                {
                    ...
                    using (UI.HorizontalScope())
                    {
                        ...
                        string[] titles = alignments.Select(a => a.Acronym().color(a.Color()).bold()).ToArray();
                ...
                {
                    UI.Space(20);
                    string[] names = enumerable.Select(sb => sb.Blueprint.GetDisplayName()).ToArray();
                    string[] titles = names.Select((name, i) => $"{name} ({enumerable.ElementAt(i).CasterLevel})").ToArray();
                    ...
                    if (enumerable.Any())
                    {
                        ...
                        var spellbook = enumerable.ElementAt(selectedSpellbook);
                        ...
                        else
                        {
                            ...
                            using (UI.HorizontalScope())
                            {
                                UI.EnumerablePicker(
                                    ...
                                    lvl =>
                                    {
                                        string levelText = lvl <= casterLevel ? $"L{lvl}".bold() : $"L{lvl}".grey();
                                        int knownCount = spellbook.GetKnownSpells(lvl).Count();
                                        string countText = knownCount > 0 ? $" ({knownCount})".white() : string.Empty;
            ...
            {
                UI.Label(string.Format("{0}{1}", $"{respecableCount} characters".yellow().bold(), " can be respecced. Pressing Respec will close the mod " +
                                                                                                  "window and take you to character level up".orange()));

                UI.Label(string.Format("{0}{1}{2}{3}", "WARNING".yellow().bold(), " this feature is ".orange(), "EXPERIMENTAL".yellow().bold(), " and uses " +
                                       "unreleased and likely buggy code.".orange()));

                UI.Label(string.Format("{0}{1}", "BACK UP".yellow().bold(), " before playing with this feature. You will lose your mythic ranks but you can " +
                                                                            "restore them in this Party Editor.".orange()));
            }
