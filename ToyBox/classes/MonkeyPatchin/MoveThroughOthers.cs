    ...
    public static class MoveThroughOthers {
        ...
        static class UnitMovementAgent_AvoidanceDisabled_Patch {
            ...
            static void Postfix(UnitMovementAgent __instance, ref bool __result) {
                var instanceUnit = __instance.Unit;

                if (instanceUnit != null && UnitEntityDataUtils.CheckUnitEntityData(instanceUnit.EntityData, settings.allowMovementThroughSelection)) {
                    __result = true;
                }
            }
        }
        ...
        static class UnitMovementAgent_IsSoftObstacle_Patch {
            ...
            static bool Prefix(UnitMovementAgent __instance, ref bool __result) {
                ...
                if (instanceUnit != null && !UnitEntityDataUtils.CheckUnitEntityData(instanceUnit.EntityData, settings.allowMovementThroughSelection)) {
                    // this duplicates the logic in the original logic for IsSoftObstacle.  If we are not in combat mode and it is not in our allow movement
                    // through category then it is a soft obstacle
