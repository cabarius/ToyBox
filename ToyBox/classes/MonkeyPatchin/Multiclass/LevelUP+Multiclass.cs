...
#if DEBUG
                        Main.Log($"SPELLBOOK: Giving {Math.Max(0, expectedCount - actual)} spells of level {index}; expected:{expectedCount} and actual:{actual}; known:{known.Count()}; special: {special}");
#endif
