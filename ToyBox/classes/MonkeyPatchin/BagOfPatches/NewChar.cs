...
namespace ToyBox.BagOfPatches
{
    static class NewChar
    {
        ...
        public static class LevelUpState_Patch
        {
            ...
            {
                if (__instance.IsFirstCharacterLevel && !__instance.IsPregen)
                {
                    // Kludge - there is some weirdness where the unit in the character generator does not return IsCustomCharacter() as true during
                    // character creation so I have to check the blueprint. The thing is if I actually try to get the blueprint name the game crashes
                    // so I do this kludge calling unit.Blueprint.ToString()
