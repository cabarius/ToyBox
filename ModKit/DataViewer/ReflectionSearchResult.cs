...
namespace ModKit.DataViewer {
    ...
    public class ResultNode<TNode> : ResultNode where TNode : class {
        ...
        public TNode Node;
        public List<ResultNode<TNode>> children = new();
        ...
        public ResultNode<TNode> FindChild(TNode node) => children.Find(rn => rn.Node == node);
        ...
        private StringBuilder BuildString(StringBuilder builder, int depth) {
            ...
            foreach (var child in children) builder = child.BuildString(builder, depth + 1);
            return builder;
        }
        public override string? ToString() => BuildString(new StringBuilder().Append("\n"), 0).ToString();
    }
    ...
    public class ReflectionSearchResult : ResultNode<Node> {
        ...
        public void AddSearchResult(Node node) {
            ...
            for (var n = node; node != null && node != Node; node = node.GetParent()) path.Add(node);
